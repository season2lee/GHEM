...
public class SwaggerConfiguration {
    ...
    public Docket UserCommentApi() {
        final ApiInfo apiInfo = new ApiInfoBuilder().title("Convenience  API")
                .description("<h3>Convenience API에 대한 문서를 제공한다.</h3>")
                .version("0.0").build();

        return new Docket(DocumentationType.SWAGGER_2) // Swagger 2.0 기반의 문서 작성
                .apiInfo(apiInfo) // 문서에 대한 정보를 설정한다.
                .groupName("b. User")
                .host("j8d107.p.ssafy.io:32001")
                .select() // ApiSelectorBuilder를 반환하며 상세한 설정 처리
                .apis(RequestHandlerSelectors.basePackage("com.ssafy.ghem.convenience"))// 대상으로하는 api 설정
